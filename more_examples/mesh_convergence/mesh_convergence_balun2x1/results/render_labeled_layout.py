#!/usr/bin/env python
"""Render the balun2x1 edge-coupled balun layout with port positions labeled.
...
201-204 -- the source_layernum values used in the model scripts):
201/202 = primary +/- (PP/PN), 203/204 = secondary +/- (S1/S2).
Port coordinates are read from a generated port_information.json (falls
back to a hardcoded default measured directly from the GDS otherwise).
"""
import os
import sys
import json
...
GDS_FILE = os.path.join(STUDY_DIR, "balun2x1_edgecoupled_do200_w8_s2.gds")
OUT_FILE = os.path.join(HERE, "plots", "balun2x1_layout_labeled.png")

PORT_NAMES = {1: "PP (pri+)", 2: "PN (pri-)", 3: "S1 (sec+)", 4: "S2 (sec-)"}

# fallback coordinates, measured directly from the GDS (layers 201-204)
FALLBACK_CENTROIDS = {1: (-116.0, 18.63), 2: (-116.0, -18.63), 3: (116.0, 18.63), 4: (116.0, -18.63)}
...
def main():
    ...
    for port_num, (cx, cy) in centroids.items():
        ...
        ax.annotate(
            PORT_NAMES[port_num],
            xy=(cx, cy),
            xytext=(cx + dx * span * 0.14, cy + dy * span * 0.09),
            color="#ffffff", fontsize=12, fontweight='bold', zorder=11,
            ha='center', va='center',
