...
**Verified against an independent tool:** cross-checked this reduction against a Keysight ADS circuit simulation (the same `.s5p` file imported as an S-parameter block, port 3 tied to ground, a real 100 Ω `Term` element bridging ports 1–2 and another bridging ports 4–5, `Zin = stoz(S11,100)`) — matches this script's output to three decimal places at 30 GHz on the 1 µm mesh (36.788+84.804j Ω both ways). An earlier version of this analysis used `R_load = 50 Ω` instead of the correct 100 Ω differential value, which is what the ADS cross-check caught; the numbers and conclusions below reflect the fix.
